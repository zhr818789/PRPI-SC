...
metrics_whole = {'PRPI-SC': np.zeros(6),
                  'Conjoint-Struct-SAE': np.zeros(6),
                  'Conjoint-Struct-CNN': np.zeros(6),
}
...
# load data settings
if DATA_SET in ['RPI369', 'RPI488', 'RPI1807', 'RPI2241', 'NPInter', 'ATH', 'ZEA']:
...
for fold in range(K_FOLD):
    ...
    print(str('\n# ' + '=' * 10 + " Fold {} " + "=" * 10 + '\n').format(fold))
    out.write(str('\n# ' + '=' * 10 + " Fold {} " + "=" * 10 + '\n').format(fold))
    model_metrics = {'PRPI-SC': np.zeros(6),
                      'Conjoint-Struct-SAE': np.zeros(6),
                      'Conjoint-Struct-CNN': np.zeros(6),
                     }
    ...
    ensemble_in = concatenate([ model_conjoint_struct_sae.output,
    ...
    model_ensemble = Model(
        inputs= model_conjoint_struct_sae.input  + model_conjoint_struct_cnn.input,
    ...
    model_metrics['PRPI-SC'] = np.array(calc_metrics(y_test[:, 1], y_test_predict[:, 1]))
    print('Best performance for module ensemble:\n {}\n'.format(model_metrics['PRPI-SC'].tolist()))
